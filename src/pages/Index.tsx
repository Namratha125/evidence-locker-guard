...
import { FolderOpen, FileText, Tag, Users, TrendingUp, Clock } from 'lucide-react';
...
const Index = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [casesData, evidenceData, tagsData, recentCasesData, recentEvidenceData] = await Promise.all([
        ...
        supabase.from('evidence').select('*, case:cases(case_number, title)').order('created_at', { ascending: false }).limit(5)
      ]);

      setStats({
        totalCases: casesData.count || 0,
        totalEvidence: evidenceData.count || 0,
        totalTags: tagsData.count || 0,
        recentCases: recentCasesData.data || [],
        recentEvidence: recentEvidenceData.data || []
      });
    } catch (error) {
            ...
            {stats.recentEvidence.slice(0, 3).map((evidence: any) => (
              <div key={evidence.id} className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{evidence.case?.case_number}</p>
                </div>
                <Badge variant="secondary">{evidence.file_type}</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
